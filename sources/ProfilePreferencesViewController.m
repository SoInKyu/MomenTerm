...
- (void)warnAboutCorruptNib {
    ...
    NSString *message;
    if (!team) {
        message = @"A required user interface component is missing or corrupted and MomenTerm\u2019s code signature could not be verified. You should download a fresh copy of the app and reinstall it.";
    } else if (![team isEqualToString:@"H7V7XYVQ7D"]) {
        message = @"A required user interface component is missing or corrupted and MomenTerm\u2019s code signature did not match that of the official distribution. You should download a fresh copy of the app and reinstall it.";
    } else {
        message = @"A required user interface component is missing or corrupted, yet against all odds the code signature for MomenTerm is valid. Please file a bug at https://github.com/SoInKyu/MomenTerm/issues";
    }
...
- (void)showDynamicProfileWarning {
    ...
    NSString *message = [NSString stringWithFormat:@"The selected profile, “%@”, is a dynamic profile. These are generally only edited by hand.\n\nMomenTerm is now able to write changes back to dynamic profiles when they are marked as “rewritable“. Rewriting can cause the order of values to change.", profileName];
